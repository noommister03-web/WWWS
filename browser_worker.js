...
async function conversations(page,b,fullScan=false){
  ...
  if(fullScan)await page.evaluate(()=>{const xs=[...document.querySelectorAll('*')].filter(e=>{const s=getComputedStyle(e);return e.scrollHeight>e.clientHeight+50&&/(auto|scroll)/.test(s.overflowY)}).sort((a,b)=>b.clientHeight-a.clientHeight);const sc=xs[0]||document.scrollingElement;sc.scrollTop=0;window.scrollTo(0,0)});
  for(let pass=0;pass<(fullScan?400:1)&&unchanged<12;pass++){
    ...
    const moved=fullScan?await page.evaluate(()=>{const xs=[...document.querySelectorAll('*')].filter(e=>{const s=getComputedStyle(e);return e.scrollHeight>e.clientHeight+50&&/(auto|scroll)/.test(s.overflowY)}).sort((a,b)=>b.clientHeight-a.clientHeight);const sc=xs[0]||document.scrollingElement,before=sc.scrollTop,max=sc.scrollHeight-sc.clientHeight;sc.scrollTop=Math.min(max,before+Math.max(300,sc.clientHeight*.8));window.scrollTo(0,document.body.scrollHeight);return{before,after:sc.scrollTop,max}}):{before:0,after:0,max:0};
    if(fullScan){await page.waitForTimeout(500);if(moved.after>=moved.max-2&&unchanged>=6)break;}
  }
...
async function messages(page,url,fullHistory=false){
  ...
  const found=new Map();let unchanged=0;
  for(let pass=0;pass<(fullHistory?600:1)&&unchanged<14;pass++){
    ...
    const moved=fullHistory?await page.evaluate(()=>{const xs=[...document.querySelectorAll('*')].filter(e=>{const s=getComputedStyle(e);return e.scrollHeight>e.clientHeight+50&&/(auto|scroll)/.test(s.overflowY)}).sort((a,b)=>b.clientHeight-a.clientHeight);const sc=xs[0]||document.scrollingElement,before=sc.scrollTop;sc.scrollTop=Math.max(0,before-Math.max(300,sc.clientHeight*.8));window.scrollTo(0,0);return{before,after:sc.scrollTop}}):{before:0,after:0};
    if(fullHistory){await page.waitForTimeout(500);const count=found.size;unchanged=(rows.length===0||count===sizeBefore)?unchanged+1:0;if(moved.after<=2&&unchanged>=8)break;}
  }
