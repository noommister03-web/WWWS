...
async function use(req,res,fn){try{const s=await session(req.body?.accountId);if(s.manualUntil>Date.now()){const e=Error("Manual CustoJusto login is in progress");e.status=423;throw e}await exclusive(s,async()=>{if(!s.automationPage||s.automationPage.isClosed())s.automationPage=await s.context.newPage();return fn(s.automationPage,base(req.body?.baseUrl))})}catch(e){if(!res.headersSent)res.status(Number(e.status)||500).json({error:e.message,status:Number(e.status)||500})}}
...
async function messages(page,url,fullHistory=false){
  ...
  if(fullHistory)for(let pass=0;pass<600&&unchanged<14;pass++){
    const pos=await scrollRelevant(page,selector,"up");await page.waitForTimeout(500);const older=await read();
    let overlap=0,max=Math.min(older.length,merged.length);for(let n=max;n>0;n--){let ok=true;for(let i=0;i<n;i++)if(token(older[older.length-n+i])!==token(merged[i])){ok=false;break}if(ok){overlap=n;break}}
    const prefix=older.slice(0,older.length-overlap);if(prefix.length){merged=prefix.concat(merged);unchanged=0}else unchanged++;
...
app.post("/manual/prepare",auth,async(req,res)=>{try{const account=id(req.body?.accountId),email=String(req.body?.email||"").trim(),password=String(req.body?.password||"");if(!email||!password)return res.status(400).json({error:"email and password are required"});const s=await session(account);s.manualUntil=Date.now()+15*60*1000;await s.page.goto(new URL("/login",base(req.body?.baseUrl)).toString(),{waitUntil:"commit",timeout:Number(process.env.CJ_BROWSER_LAUNCH_TIMEOUT_MS||120000)});await cookies(s.page);const emailField=s.page.locator("#username");const passwordField=s.page.locator("#password");await emailField.waitFor({state:"visible",timeout:Number(process.env.CJ_BROWSER_LAUNCH_TIMEOUT_MS||120000)});await emailField.fill(email);await passwordField.waitFor({state:"visible",timeout:Number(process.env.CJ_BROWSER_LAUNCH_TIMEOUT_MS||120000)});await passwordField.fill(password);res.json({ok:true})}catch(e){res.status(500).json({error:e.message})}});
...
app.post("/conversations",auth,(req,res)=>use(req,res,async(p,b)=>{if(!await logged(p))return res.status(401).json({error:"CustoJusto session is not logged in"});res.json(await conversations(p,b,req.body?.fullScan===true))}));
app.post("/messages",auth,(req,res)=>use(req,res,async(p)=>{if(!await logged(p))return res.status(401).json({error:"CustoJusto session is not logged in"});const u=String(req.body?.conversationUrl||"");if(!u)return res.status(400).json({error:"conversationUrl is required"});res.json(await messages(p,u,req.body?.fullHistory===true))}));
