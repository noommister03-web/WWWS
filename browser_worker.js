...
async function use(req,res,fn){try{const s=await session(req.body?.accountId);await exclusive(s,async()=>{if(!s.automationPage||s.automationPage.isClosed())s.automationPage=await s.context.newPage();return fn(s.automationPage,base(req.body?.baseUrl))})}catch(e){if(!res.headersSent)res.status(Number(e.status)||500).json({error:e.message,status:Number(e.status)||500})}}
