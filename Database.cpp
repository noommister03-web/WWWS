...
bool Database::hasCustoJustoDraftForSource(
    long long accountId,
    long long conversationId,
    const std::string& sourceExternalMessageId
) {
    if (sourceExternalMessageId.empty()) {
        return false;
    }
    sqlite3_stmt* statement = nullptr;
    const char* sql =
        "SELECT 1 FROM custojusto_drafts "
        "WHERE account_id=? AND conversation_id=? "
        "AND source_external_message_id=? LIMIT 1;";
    int rc = sqlite3_prepare_v2(db_, sql, -1, &statement, nullptr);
    checkSqlite(rc, db_, "prepare source draft check");
    sqlite3_bind_int64(statement, 1, accountId);
    sqlite3_bind_int64(statement, 2, conversationId);
    sqlite3_bind_text(
        statement, 3, sourceExternalMessageId.c_str(), -1, SQLITE_TRANSIENT
    );
