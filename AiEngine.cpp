...
std::string AiEngine::generateReply(const std::vector<MessageRecord>& history,const std::string& overridePrompt)const{
 if(!enabled())return"";json messages=json::array();messages.push_back({{"role","system"},{"content",overridePrompt.empty()?systemPrompt_:overridePrompt}});size_t max=std::min<size_t>(history.size(),30),start=history.size()-max;for(size_t i=start;i<history.size();++i)if(!history[i].text.empty())messages.push_back({{"role",history[i].incoming?"user":"assistant"},{"content",limitUtf8(history[i].text,6000)}});
