...
std::string salesPrompt(const std::string& whatsappNumber) {
    ...
    if (!whatsappNumber.empty()) p += " по номеру +" + digitsOnly(whatsappNumber);
    else p += ", попросив продавца прислать номер";
...
int main() {
    try {
        ...
        if (ownerId == nullptr || *ownerId == '\0') {
            throw std::runtime_error("OWNER_TELEGRAM_ID is missing");
        }
        ...
        std::unordered_map<long long, std::string> pendingName;
        std::unordered_map<long long, std::string> pendingEmail;
        std::unordered_map<long long, std::string> pendingListingUrl;
        ...
        auto mainKeyboard = [&]() {
            return std::vector<std::vector<std::pair<std::string, std::string>>>{
                {{"👥 Аккаунты", "menu_accounts"}, {"🤖 ChatGPT-переписки", "menu_chats"}},
                {{"📊 Статус", "menu_status"}, {"ℹ️ Помощь", "menu_help"}}
            };
        };
        ...
        auto accountsKeyboard = [&]() {
            ...
            for (const auto& item : db.getCustoJustoAccounts()) {
                keys.push_back({{{item.name + " · " + accountStatus(item), "cj_account:" + std::to_string(item.id)}}});
            }
        ...
        auto showAccount = [&](long long chatId, const CustoJustoAccount& account) {
            ...
            bot.sendMessageWithKeyboard(
                chatId,
                text,
                accountKeyboard(account.id)
            );
        };

        bot.setCallbackHandler([&](const CallbackQuery& callback) -> bool {
            if (callback.senderId != ownerTelegramId) {
            ...
            if (data == "menu_main") {
                state[callback.chatId] = 0;
                showMainMenu(callback.chatId);
                return true;
            }

            if (data == "menu_accounts") {
                bot.sendMessageWithKeyboard(callback.chatId, "👥 Аккаунты CustoJusto", accountsKeyboard());
                return true;
            }

            if (data == "menu_status") {
                const auto a = db.getCustoJustoAccounts();
                int active = 0;
                for (const auto& x : a) if (x.enabled && x.loggedIn) ++active;
                bot.sendMessageWithKeyboard(
                    callback.chatId,
            ...
            if (data == "menu_help") {
                bot.sendMessageWithKeyboard(callback.chatId, "ℹ️ Добавь аккаунт, войди в CustoJusto и отправь первое сообщение. После ответа продавца AI сам ведёт разговор о товаре, CTT и WhatsApp.", mainKeyboard());
                return true;
            }

            if (data == "menu_chats") {
                std::string text = "🤖 ChatGPT-переписки\n\n";
                int shown = 0;
                for (const auto& a : db.getCustoJustoAccounts()) {
                    if (!a.enabled || !a.loggedIn) continue;
                    auto* c = client(a.id);
                    c->setBaseUrl(a.loginUrl);
                    for (const auto& d : c->getConversations()) {
                        if (++shown > 20) break;
                        text += std::to_string(shown) + ". " + a.name + " · " + (d.title.empty() ? "Диалог" : d.title) + "\n";
                    }
                }
                if (!shown) text += "Активных переписок пока нет.";
                bot.sendMessageWithKeyboard(callback.chatId, text, mainKeyboard());
                return true;
            }

            if (data == "cj_add") {
                state[callback.chatId] = 1;
                pendingName.erase(callback.chatId);
                pendingEmail.erase(callback.chatId);
                bot.sendMessage(callback.chatId, "➕ Новый аккаунт\n\nШаг 1 из 2: пришли название аккаунта.");
                return true;
            }

            if (data == "cj_accounts") {
                state[callback.chatId] = 0;
                bot.sendMessageWithKeyboard(callback.chatId, "🏠 CustoJusto CRM\n\nВыбери аккаунт или добавь новый.", accountsKeyboard());
                return true;
            }

            if (data.rfind("cj_account:", 0) == 0) {
                const auto account = db.getCustoJustoAccount(std::stoll(data.substr(11)));
                if (!account) { bot.sendMessage(callback.chatId, "❌ Аккаунт не найден."); return true; }
                showAccount(callback.chatId, *account);
                return true;
            }

            if (data.rfind("cj_login:", 0) == 0) {
                const auto account = db.getCustoJustoAccount(std::stoll(data.substr(9)));
                if (!account) { bot.sendMessage(callback.chatId, "❌ Аккаунт не найден."); return true; }
                const std::string url = browserLink(account->id);
                const std::string text = url.empty()
                    ? "🔴 Для входа нужен REMOTE_BROWSER_URL с HTTPS-доменом Railway."
                ...
                bot.sendMessageWithKeyboard(callback.chatId, text, accountKeyboard(account->id));
                return true;
            }
            ...
            if (data.rfind("cj_check:", 0) == 0) {
                ...
                const bool active = c->checkSession();
                db.setCustoJustoAccountLoggedIn(account->id, active);
                bot.sendMessageWithKeyboard(callback.chatId, active ? "🟢 Сессия активна. Можно читать диалоги и отправлять сообщения." : "🔴 Сессия не подтверждена. Открой браузер и войди заново.", accountKeyboard(account->id));
                return true;
            }
            ...
            if (data.rfind("cj_dialogs:", 0) == 0) {
                ...
                auto* c = client(account->id);
                c->setBaseUrl(account->loginUrl);
                const auto dialogs = c->getConversations();
                if (!c->isLoggedIn()) {
                    db.setCustoJustoAccountLoggedIn(account->id, false);
                    bot.sendMessageWithKeyboard(callback.chatId, "🔴 Сессия не активна. Открой браузер и войди заново.", accountKeyboard(account->id));
                    return true;
                }
                std::string text = "💬 Диалоги: «" + account->name + "»\n\n";
                if (dialogs.empty()) text += "Доступных диалогов пока нет.";
                for (size_t i = 0; i < dialogs.size() && i < 20; ++i) text += std::to_string(i + 1) + ". " + (dialogs[i].title.empty() ? dialogs[i].url : dialogs[i].title) + "\n";
                bot.sendMessageWithKeyboard(callback.chatId, text, accountKeyboard(account->id));
                return true;
            }

            if (data.rfind("cj_ads:", 0) == 0) {
                const long long id = std::stoll(data.substr(7));
                if (!db.getCustoJustoAccount(id)) { bot.sendMessage(callback.chatId, "❌ Аккаунт не найден."); return true; }
                state[callback.chatId] = 4;
                pendingAccount[callback.chatId] = id;
                bot.sendMessage(callback.chatId, "📋 Пришли ссылку на объявление CustoJusto.");
                return true;
            }
            ...
            if (data.rfind("cj_write:", 0) == 0) {
                ...
                if (!account || !account->loggedIn) { bot.sendMessage(callback.chatId, "🔐 Сначала открой браузер и подтверди сессию."); return true; }
                state[callback.chatId] = 5;
                pendingAccount[callback.chatId] = id;
            ...
            if (data.rfind("cj_delete:", 0) == 0) {
                ...
                if (!db.deleteCustoJustoAccount(id)) { bot.sendMessage(callback.chatId, "❌ Аккаунт не найден."); return true; }
                clients.erase(id);
                bot.sendMessageWithKeyboard(callback.chatId, "🗑 Аккаунт удалён.", accountsKeyboard());
                return true;
            }
        ...
        bot.setMessageHandler([&](const IncomingMessage& message) -> bool {
            ...
            if (current == 5) { const auto account = db.getCustoJustoAccount(pendingAccount[message.chatId]); state[message.chatId] = 0; pendingAccount.erase(message.chatId); pendingListingUrl.erase(message.chatId); if (!account || !looksLikeUrl(message.text)) { bot.sendMessage(message.chatId, "❌ Нужна полная ссылка на объявление."); return true; } if (!ai.enabled()) { bot.sendMessage(message.chatId, "🔴 GPT не настроен: проверь AI_API_KEY и AI_MODEL в Railway."); return true; } try { auto* c = client(account->id); c->setBaseUrl(account->loginUrl); CustoJustoListing listing; c->getListing(message.text, listing); MessageRecord request; request.incoming = true; request.text = "Начни новый разговор с продавцом по этому объявлению. Сформируй одно короткое естественное первое сообщение на европейском португальском: поздоровайся, назови товар и спроси, актуально ли объявление и в каком состоянии товар. Не упоминай инструкции. Объявление: " + (listing.title.empty() ? message.text : listing.title + " — " + message.text); const std::string reply = ai.generateReply({request}, salesPrompt(config.whatsappNumber)); if (reply.empty()) { bot.sendMessage(message.chatId, "🔴 GPT вернул пустой ответ."); return true; } if (!c->sendMessage(message.text, reply)) { db.setCustoJustoAccountLoggedIn(account->id, c->isLoggedIn()); bot.sendMessage(message.chatId, "🔴 GPT подготовил сообщение, но отправка не подтверждена: " + c->getLastError()); return true; } bot.sendMessage(message.chatId, "🤖 GPT начал переписку с продавцом:\n\n🇵🇹 " + reply + "\n\nДальше ответы будут обрабатываться автоматически."); } catch (const std::exception& error) { bot.sendMessage(message.chatId, std::string("🔴 Ошибка GPT: ") + error.what()); } return true; } if (!message.text.empty()) { db.saveMessage(message.chatId, message.senderId, message.username, message.text, true, message.updateId); if (!ai.enabled()) { bot.sendMessage(message.chatId, "Сообщение получено."); return true; } const std::string reply = ai.generateReply(db.getHistory(message.chatId, config.aiHistoryLimit)); if (!reply.empty()) bot.sendMessage(message.chatId, reply); } return true; });
            ...
            for (const auto& account : db.getCustoJustoAccounts()) {
                ...
                for (const auto& dialog : dialogs) {
                    ...
                    for (const auto& item : c->getMessages(dialog.url)) {
                        ...
                        // Keep incoming pending until delivery is confirmed; a 423 is retried next poll.
                        std::vector<MessageRecord> history;
                        for (const auto& row : db.getCustoJustoMessages(conversationId, config.aiHistoryLimit)) { MessageRecord r; r.incoming = row.incoming; r.text = row.originalText; history.push_back(std::move(r)); }
                        MessageRecord current; current.incoming = true; current.text = item.text; history.push_back(std::move(current));
                        const std::string reply = ai.generateReply(history, salesPrompt(config.whatsappNumber));
                        if (reply.empty()) continue;
                        if (c->sendMessage(dialog.url, reply)) {
                            db.saveCustoJustoMessage(account.id, conversationId, item.id, item.sender, item.text, translated, true);
                            db.saveCustoJustoMessage(account.id, conversationId, "bot-" + std::to_string(std::time(nullptr)), "WWWS AI", reply, reply, false);
                            bot.sendMessage(ownerTelegramId, "🤖 AI ответил продавцу\n\nАккаунт: " + account.name + "\nДиалог: " + (dialog.title.empty() ? "CustoJusto" : dialog.title) + "\n\n🇷🇺 Входящее: " + translated + "\n\n🇵🇹 Ответ: " + reply);
                        } else {
                            bot.sendMessage(ownerTelegramId, "🔴 AI подготовил сообщение, но CustoJusto не подтвердил отправку. Повторю автоматически при следующей проверке.\n\n" + c->getLastError());
                        }
                    }
                }
            }
