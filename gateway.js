...
async function worker(path, options = {}) {
  const response = await fetch(`http://127.0.0.1:3001${path}`, { ...options, headers: { "x-worker-secret": WORKER_SECRET, ...(options.headers || {}) } });
  const redirect = options.redirect === "manual" && response.status >= 300 && response.status < 400;
  if (!response.ok && !redirect) throw new Error((await response.text()).slice(0, 250) || "Browser worker request failed.");
  return response;
}
