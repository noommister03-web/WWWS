...
function cookies(header){return Object.fromEntries(String(header||"").split(";").map(part=>{const i=part.indexOf("=");return i<0?["",""]:[part.slice(0,i).trim(),decodeURIComponent(part.slice(i+1).trim())];}).filter(([key])=>key));}
function session(req){const token=cookies(req.headers.cookie).cj_browser_session,expiresAt=sessions.get(token);if(!expiresAt||expiresAt<Date.now()){if(token)sessions.delete(token);return false;}return true;}
function passwordMatches(value){const given=Buffer.from(String(value||"")),expected=Buffer.from(PASSWORD);return expected.length>0&&given.length===expected.length&&crypto.timingSafeEqual(given,expected);}
function loginKey(req){return String(req.ip||req.socket.remoteAddress||"unknown");}
function loginState(req){const key=loginKey(req),now=Date.now();let state=loginAttempts.get(key);if(!state||(now>=state.lockedUntil&&now-state.windowStartedAt>=LOGIN_WINDOW_MS)){state={windowStartedAt:now,failures:0,lockedUntil:0};loginAttempts.set(key,state);}return{key,state,now};}
function loginLocked(req){const{state,now}=loginState(req);return state.lockedUntil>now;}
function recordLoginFailure(req){const{state,now}=loginState(req);state.failures+=1;if(state.failures>=MAX_LOGIN_FAILURES)state.lockedUntil=Math.max(state.lockedUntil,now+LOGIN_LOCK_MS);}
...
async function currentStatus(account){const response=await worker("/status",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({accountId:account})});return response.json();}
...
setInterval(()=>{const now=Date.now();for(const[token,until]of sessions)if(until<now)sessions.delete(token);for(const[key,state]of loginAttempts)if(now>=state.lockedUntil&&now-state.windowStartedAt>=LOGIN_WINDOW_MS)loginAttempts.delete(key);},60000).unref();
app.get("/health",(_,res)=>res.type("text/plain").send("ok"));
app.get("/browser-api/manual/open",async(req,res)=>{let account="";try{account=accountId(req.query.accountId);if(!session(req))return res.type("html").send(accessForm(account));try{const state=await currentStatus(account);if(state.loggedIn)return res.redirect(302,"/vnc.html?autoconnect=true&resize=scale&show_dot=true");}catch(error){console.error(`Unable to check existing CustoJusto session: ${error.message}`);}res.type("html").send(form(account));}catch(error){res.status(400).type("text/plain").send(error.message);}});
app.post("/browser-api/access",(req,res)=>{try{const account=accountId(req.body.accountId);if(loginLocked(req))return res.status(429).type("html").send(accessForm(account,"Слишком много попыток. Попробуй позже."));if(!passwordMatches(req.body.browserPassword)){recordLoginFailure(req);return res.status(401).type("html").send(accessForm(account,"Неверный пароль удалённого браузера."));}clearLoginFailures(req);createSession(res);res.redirect(303,`/browser-api/manual/open?accountId=${encodeURIComponent(account)}&mobile=1`);}catch(error){res.status(400).type("text/plain").send(error.message);}});
app.post("/browser-api/manual/open",requireSession,async(req,res)=>{let account="";try{account=accountId(req.body.accountId);const email=String(req.body.email||"").trim(),password=String(req.body.password||"");if(!email||!password)return res.status(400).type("html").send(form(account,"Заполни email и пароль."));try{const state=await currentStatus(account);if(state.loggedIn)return res.redirect(302,"/vnc.html?autoconnect=true&resize=scale&show_dot=true");}catch(_){}await worker("/manual/prepare",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({accountId:account,email,password})});res.redirect(302,"/vnc.html?autoconnect=true&resize=scale&show_dot=true");}catch(error){res.status(502).type("html").send(form(account,`Не удалось открыть браузер: ${error.message}`));}});
