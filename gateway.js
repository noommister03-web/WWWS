...
app.post("/browser-api/access", (req, res) => { try { const account = accountId(req.body.accountId); if (!passwordMatches(req.body.browserPassword)) return res.status(401).type("html").send(accessForm(account, "Неверный пароль удалённого браузера.")); createSession(res); res.redirect(303, `/browser-api/manual/open?accountId=${encodeURIComponent(account)}&mobile=1`); } catch (error) { res.status(400).type("text/plain").send(error.message); } });
